...
let closureWorkflowConfig = {};
let closureWorkflowSteps = [];
...
async function renderTabClosures(content, h) {
    ...
    try {
        const wfRes = await API.get(`closures/workflow-config/${h.id}`);
        closureWorkflowConfig = wfRes.config || {};
        closureWorkflowSteps = wfRes.steps || [];
    } catch (e) {
        closureWorkflowConfig = {};
        closureWorkflowSteps = [];
    }

    // Charger la liste des hôtels pour la réplication
    let allHotels = [];
    try {
        const hRes = await API.getHotels();
        allHotels = (hRes.hotels || []).filter(ht => ht.id !== h.id);
    } catch (e) { allHotels = []; }

    const ROLE_OPTIONS = [
        { value: 'admin', label: 'Administrateur' },
        { value: 'groupe_manager', label: 'Responsable Groupe' },
        { value: 'hotel_manager', label: 'Responsable Hôtel' },
        { value: 'comptabilite', label: 'Comptabilité' },
        { value: 'receptionniste', label: 'Réceptionniste' }
    ];

    const wf = closureWorkflowConfig;
    const validatorRoles = wf.validator_roles || ['admin', 'groupe_manager'];
    const validatorRolesL2 = wf.validator_roles_level2 || ['admin'];

    content.innerHTML = `
        <!-- SECTION WORKFLOW DE VALIDATION -->
        <div class="card mb-20">
            <div class="card-header">
                <h3 class="card-title"><i class="fas fa-project-diagram"></i> Workflow de validation</h3>
                ${API.user.role === 'admin' && allHotels.length > 0 ? `
                    <button type="button" class="btn btn-sm btn-outline" onclick="showWorkflowReplicateModal(${h.id})">
                        <i class="fas fa-copy"></i> Répliquer vers un autre hôtel
                    </button>
                ` : ''}
            </div>
            <div class="card-body" style="padding:24px">
                <p class="text-muted mb-20">Configurez les seuils de validation, les rôles autorisés et les étapes du workflow pour cet hôtel.</p>

                <!-- Seuils de montant -->
                <h4 style="margin-bottom:12px"><i class="fas fa-sliders-h"></i> Seuils de montant</h4>
                <div class="form-row mb-20">
                    <div class="form-group" style="flex:1">
                        <label>Auto-validation si dépenses ≤</label>
                        <div class="input-group">
                            <input type="number" step="0.01" min="0" class="form-control" id="wf-auto-validate-limit"
                                   value="${esc(wf.auto_validate_limit || '0')}" placeholder="0">
                            <span class="input-suffix">€</span>
                        </div>
                        <small class="form-help">0 = désactivé. Les clôtures sous ce seuil sont auto-validées.</small>
                    </div>
                    <div class="form-group" style="flex:1">
                        <label>Validation manager si dépenses ≤</label>
                        <div class="input-group">
                            <input type="number" step="0.01" min="0" class="form-control" id="wf-manager-validate-limit"
                                   value="${esc(wf.manager_validate_limit || '0')}" placeholder="0">
                            <span class="input-suffix">€</span>
                        </div>
                        <small class="form-help">0 = illimité. Le hotel_manager peut valider sous ce seuil.</small>
                    </div>
                </div>

                <div class="form-row mb-20">
                    <div class="form-group" style="flex:1">
                        <label>Justificatif obligatoire si dépenses ></label>
                        <div class="input-group">
                            <input type="number" step="0.01" min="0" class="form-control" id="wf-receipt-required"
                                   value="${esc(wf.receipt_required_above || '0')}" placeholder="0">
                            <span class="input-suffix">€</span>
                        </div>
                        <small class="form-help">0 = toujours obligatoire dès qu'il y a des dépenses.</small>
                    </div>
                    <div class="form-group" style="flex:1">
                        <label>Commentaire obligatoire si dépenses ></label>
                        <div class="input-group">
                            <input type="number" step="0.01" min="0" class="form-control" id="wf-comment-required"
                                   value="${esc(wf.comment_required_above || '0')}" placeholder="0">
                            <span class="input-suffix">€</span>
                        </div>
                        <small class="form-help">0 = toujours obligatoire dès qu'il y a des dépenses.</small>
                    </div>
                ...
                <div class="form-row mb-20">
                    <div class="form-group" style="flex:1">
                        <label>Double validation si dépenses ></label>
                        <div class="input-group">
                            <input type="number" step="0.01" min="0" class="form-control" id="wf-double-validation"
                                   value="${esc(wf.double_validation_above || '0')}" placeholder="0">
                            <span class="input-suffix">€</span>
                        </div>
                        <small class="form-help">0 = désactivé. Deux validations requises au-dessus de ce seuil.</small>
                    </div>
                    <div class="form-group" style="flex:1"></div>
                </div>

                <!-- Rôles de validation -->
                <h4 style="margin-bottom:12px"><i class="fas fa-user-shield"></i> Rôles autorisés à valider</h4>
                <div class="form-row mb-20">
                    <div class="form-group" style="flex:1">
                        <label>Validation niveau 1</label>
                        <div class="checkbox-group" id="wf-validator-roles">
                            ${ROLE_OPTIONS.map(r => `
                                <label class="checkbox-label">
                                    <input type="checkbox" value="${r.value}" ${validatorRoles.includes(r.value) ? 'checked' : ''}>
                                    ${r.label}
                                </label>
                            `).join('')}
                        </div>
                    ...
                    <div class="form-group" style="flex:1">
                        <label>Validation niveau 2 (double validation)</label>
                        <div class="checkbox-group" id="wf-validator-roles-l2">
                            ${ROLE_OPTIONS.map(r => `
                                <label class="checkbox-label">
                                    <input type="checkbox" value="${r.value}" ${validatorRolesL2.includes(r.value) ? 'checked' : ''}>
                                    ${r.label}
                                </label>
                            `).join('')}
                        </div>
                ...
                <!-- Alertes -->
                <h4 style="margin-bottom:12px"><i class="fas fa-bell"></i> Alertes</h4>
                <div class="form-row mb-20">
                    <div class="form-group" style="flex:1">
                        <label>Alerte si non soumise après</label>
                        <div class="input-group">
                            <input type="number" min="1" class="form-control" id="wf-alert-delay"
                                   value="${esc(wf.alert_delay_hours || '13')}" placeholder="13">
                            <span class="input-suffix">heures</span>
                        </div>
                    </div>
                    <div class="form-group" style="flex:1">
                        <label>Alerte si non validée après</label>
                        <div class="input-group">
                            <input type="number" min="1" class="form-control" id="wf-validation-deadline"
                                   value="${esc(wf.validation_deadline_hours || '48')}" placeholder="48">
                            <span class="input-suffix">heures</span>
                        </div>
                    </div>
                </div>
                ...
                <h4 style="margin-bottom:12px"><i class="fas fa-list-ol"></i> Étapes de validation personnalisées <span class="text-muted" style="font-size:12px;font-weight:normal">(optionnel)</span></h4>
                <p class="text-muted mb-15" style="font-size:13px">Définissez des étapes séquentielles de validation. Si aucune étape n'est définie, le workflow simple (validation directe) s'applique.</p>

                <div id="wf-steps-container">
                    ${renderWorkflowSteps()}
                </div>

                <button type="button" class="btn btn-outline btn-block mt-10 mb-20" onclick="wfAddStep()">
                    <i class="fas fa-plus"></i> Ajouter une étape
                </button>

                <div style="text-align:right;margin-top:20px">
                    <button type="button" class="btn btn-primary" onclick="saveWorkflowConfig(${h.id})">
        ...
        <!-- SECTION DOCUMENTS (existante) -->
        <div class="card">
            <div class="card-header">
                <h3 class="card-title"><i class="fas fa-file-alt"></i> Documents de clôture</h3>
            </div>
            <div class="card-body" style="padding:24px">
                ...
                <div style="text-align:right;margin-top:20px">
                    <button type="button" class="btn btn-primary" onclick="saveClosureConfig(${h.id})">
                        <i class="fas fa-save"></i> Enregistrer les documents
                    </button>
                </div>
            </div>
        </div>

        <!-- Modal réplication (hidden) -->
        <div id="workflow-replicate-data" style="display:none" data-hotels='${esc(JSON.stringify(allHotels))}'></div>
    `;
}
...
function renderWorkflowSteps() {
    if (closureWorkflowSteps.length === 0) {
        return `
            <div class="empty-state" style="padding:20px">
                <i class="fas fa-route"></i>
                <p>Aucune étape définie — le workflow simple (validation directe) s'applique.</p>
            </div>
        `;
    }

    return closureWorkflowSteps.map((step, idx) => `
        <div class="card mb-10 wf-step-card" data-index="${idx}" style="border-left:3px solid var(--primary-500)">
            <div class="card-body" style="padding:12px 16px">
                <div class="form-row" style="align-items:center">
                    <div style="flex:0 0 40px;text-align:center;font-weight:bold;color:var(--primary-500);font-size:18px">
                        ${idx + 1}
                    </div>
                    <div class="form-group" style="flex:2;margin-bottom:0">
                        <input type="text" class="form-control" value="${esc(step.step_name || '')}"
                            onchange="wfUpdateStep(${idx}, 'step_name', this.value)"
                            placeholder="Nom de l'étape (ex: Validation réceptionniste)">
                    </div>
                    <div class="form-group" style="flex:1;margin-bottom:0">
                        <select class="form-control" onchange="wfUpdateStep(${idx}, 'required_role', this.value)">
                            ${WF_ROLE_OPTIONS.map(r => `
                                <option value="${r.value}" ${step.required_role === r.value ? 'selected' : ''}>${r.label}</option>
                            `).join('')}
                        </select>
                    </div>
                    <div class="form-group" style="flex:0.7;margin-bottom:0">
                        <input type="number" step="0.01" min="0" class="form-control" value="${step.min_amount || 0}"
                            onchange="wfUpdateStep(${idx}, 'min_amount', this.value)"
                            placeholder="Min €" title="Montant minimum (0 = toujours)">
                    </div>
                    <div class="form-group" style="flex:0.7;margin-bottom:0">
                        <input type="number" step="0.01" min="0" class="form-control" value="${step.max_amount || 0}"
                            onchange="wfUpdateStep(${idx}, 'max_amount', this.value)"
                            placeholder="Max €" title="Montant maximum (0 = illimité)">
                    </div>
                    <div style="flex:0 0 auto">
                        <button type="button" class="btn btn-sm btn-danger" onclick="wfRemoveStep(${idx})">
...
function wfAddStep() {
    closureWorkflowSteps.push({
        step_name: '',
        required_role: 'hotel_manager',
        min_amount: 0,
        max_amount: 0
    });
    document.getElementById('wf-steps-container').innerHTML = renderWorkflowSteps();
}

function wfRemoveStep(idx) {
    if (!confirm('Supprimer cette étape ?')) return;
    closureWorkflowSteps.splice(idx, 1);
    document.getElementById('wf-steps-container').innerHTML = renderWorkflowSteps();
}

function wfUpdateStep(idx, field, value) {
    if (field === 'min_amount' || field === 'max_amount') {
        closureWorkflowSteps[idx][field] = parseFloat(value) || 0;
    } else {
        closureWorkflowSteps[idx][field] = value;
    }
}

async function saveWorkflowConfig(hotelId) {
    // Collecter les valeurs des seuils
    const config = {
        auto_validate_limit: parseFloat(document.getElementById('wf-auto-validate-limit')?.value) || 0,
        manager_validate_limit: parseFloat(document.getElementById('wf-manager-validate-limit')?.value) || 0,
        receipt_required_above: parseFloat(document.getElementById('wf-receipt-required')?.value) || 0,
        comment_required_above: parseFloat(document.getElementById('wf-comment-required')?.value) || 0,
        double_validation_above: parseFloat(document.getElementById('wf-double-validation')?.value) || 0,
        alert_delay_hours: parseInt(document.getElementById('wf-alert-delay')?.value) || 13,
        validation_deadline_hours: parseInt(document.getElementById('wf-validation-deadline')?.value) || 48,
        validator_roles: [],
        validator_roles_level2: []
    };

    // Collecter les rôles cochés
    document.querySelectorAll('#wf-validator-roles input[type="checkbox"]:checked').forEach(cb => {
        config.validator_roles.push(cb.value);
    });
    document.querySelectorAll('#wf-validator-roles-l2 input[type="checkbox"]:checked').forEach(cb => {
        config.validator_roles_level2.push(cb.value);
    });

    if (config.validator_roles.length === 0) {
        toast('Sélectionnez au moins un rôle pour la validation niveau 1', 'warning');
        return;
    }

    // Valider les étapes
    const steps = closureWorkflowSteps.filter(s => s.step_name && s.step_name.trim());

    try {
        await API.post(`closures/workflow-config/${hotelId}`, { config, steps });
        toast('Workflow de validation enregistré', 'success');
    } catch (e) {
        toast(e.message, 'error');
    }
}

function showWorkflowReplicateModal(sourceHotelId) {
    const dataEl = document.getElementById('workflow-replicate-data');
    if (!dataEl) return;
    let hotels = [];
    try { hotels = JSON.parse(dataEl.dataset.hotels); } catch (e) { hotels = []; }

    if (hotels.length === 0) {
        toast('Aucun autre hôtel disponible', 'warning');
        return;
    }

    const html = `
        <div style="padding:10px">
            <p class="mb-15">Sélectionnez les hôtels vers lesquels répliquer le workflow de validation de <strong>cet hôtel</strong>.</p>
            <p class="text-muted mb-15" style="font-size:13px"><i class="fas fa-exclamation-triangle text-warning"></i> La configuration existante des hôtels cibles sera remplacée.</p>

            <div class="form-group">
                <label class="checkbox-label mb-10" style="font-weight:600">
                    <input type="checkbox" id="wf-replicate-all" onchange="wfReplicateToggleAll(this.checked)">
                    Tous les hôtels
                </label>
                <div id="wf-replicate-hotels" style="max-height:300px;overflow-y:auto;padding-left:10px">
                    ${hotels.map(h => `
                        <label class="checkbox-label mb-5">
                            <input type="checkbox" class="wf-replicate-hotel-cb" value="${h.id}">
                            ${esc(h.name)}
                        </label>
                    `).join('')}
                </div>
            ...
            <div style="text-align:right;margin-top:20px">
                <button type="button" class="btn btn-outline" onclick="closeModal()">Annuler</button>
                <button type="button" class="btn btn-primary ml-10" onclick="executeWorkflowReplicate(${sourceHotelId})">
                    <i class="fas fa-copy"></i> Répliquer
                </button>
            </div>
        </div>
    `;

    openModal('Répliquer le workflow', html, 'modal-md');
}

function wfReplicateToggleAll(checked) {
    document.querySelectorAll('.wf-replicate-hotel-cb').forEach(cb => cb.checked = checked);
}

async function executeWorkflowReplicate(sourceHotelId) {
    const targetIds = [];
    document.querySelectorAll('.wf-replicate-hotel-cb:checked').forEach(cb => {
        targetIds.push(parseInt(cb.value));
    });

    if (targetIds.length === 0) {
        toast('Sélectionnez au moins un hôtel cible', 'warning');
        return;
    }

    if (!confirm(`Répliquer le workflow vers ${targetIds.length} hôtel(s) ? La config existante sera remplacée.`)) return;

    try {
        const res = await API.post(`closures/workflow-config/${sourceHotelId}/replicate`, {
            target_hotel_ids: targetIds
        });
        toast(`Workflow répliqué vers ${res.replicated_count} hôtel(s)`, 'success');
        closeModal();
    } catch (e) {
        toast(e.message, 'error');
    }
